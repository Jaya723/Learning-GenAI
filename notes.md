...
17. ChromaDB expects exactly these fields: ids, embeddings, documents (optional but commonly used), metadatas (optional). Even the results format is fixed we only provide the data info that we need but if we don't provide that we would not get that simple.
